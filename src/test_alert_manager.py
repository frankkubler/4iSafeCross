...
async def test_multi_zone():
    ...
    await am.on_detection(now, detections=[{"x_min": 0, "y_min": 0, "x_max": 1, "y_max": 1, "confidence": 0.9, "zones": ["zone1"]}])
    await am.on_detection(now+1, detections=[{"x_min": 0, "y_min": 0, "x_max": 1, "y_max": 1, "confidence": 0.9, "zones": ["zone3"]}])
    # 2. Activation zone2 (relais indépendant)
    await am.on_detection(now+2, detections=[{"x_min": 0, "y_min": 0, "x_max": 1, "y_max": 1, "confidence": 0.9, "zones": ["zone2"]}])
    ...
    await am.on_detection(now+6, detections=[{"x_min": 0, "y_min": 0, "x_max": 1, "y_max": 1, "confidence": 0.9, "zones": ["zone1"]}])
    ...
    await asyncio.sleep(1)
    await am.on_detection(now+10, detections=[{"x_min": 0, "y_min": 0, "x_max": 1, "y_max": 1, "confidence": 0.9, "zones": ["zone2"]}])
