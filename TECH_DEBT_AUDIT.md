...
- **F004** — **RESOLVED (2026-09-01, `CYBER_AUDIT.md` rév. 10, `CS-1144-01`)** :
  ...
  **obligatoire**. `require_auth_config()` lève une `RuntimeError` si
  ...
  et `.env.example` sont alignés. **Rév. 12** : anti-CSRF (contrôle d'`Origin`)
  + en-têtes de sécurité CSP/`X-Frame-Options`/`COOP`… (`CS-R4-01`) ; journal
  d'audit JSON des écritures et rejets → `logs/audit.log` (`CS-144-01` /
  `CS-R2-03`). Rôles : **rôle unique par conception** (pas d'opérateur logiciel,
  `CS-113-02`) — dérogation à formaliser, pas de RBAC à écrire.
...
| F004 | Security | `src/web/app_factory.py` | Critical | L | ~~No authentication…on any of the 32 registered routes.~~ **RESOLVED (rév. 10 & 12)**: HTTP Basic auth mandatory (`require_auth_config()` raises at boot if `SAFECROSS_AUTH_*` unset; `before_request` covers every route except `static`/`/health`); `waitress` binds `127.0.0.1` only, Caddy TLS on `eth2`. Rév. 12: Origin-based CSRF guard on writes, CSP + security headers (app + Caddy), JSON audit log of writes and 401/403 rejections → `logs/audit.log`. Single role is by design (no shop-floor operator, `CS-113-02`) — derogation, not RBAC. | Done. Recette: `curl → 401` and a write both land in `logs/audit.log`. |
...
| F042 | Config | `pyproject.toml` | Medium | S | `[dependency-groups] dev` now exists (bandit/pip-audit/pytest/ruff/nuitka pinned, CI uses `uv sync --only-group dev`). **Rév. 14 (`CS-128-01`)**: `setuptools` removed from `[project.dependencies]`; `cython`/`setuptools`/`wheel` are installed ad hoc in the Dockerfile builder for the Cython step then `uv pip uninstall`ed before the final `COPY .venv`. Remaining smell: `cython==3.2.8` pinned in the `Dockerfile` line rather than a lockfile-tracked build group. | Optional: a `build` dependency-group for `cython`/`wheel` so their versions are lock-tracked. |
| F043 | CI | `.gitlab-ci.yml:24-33,40` | Medium | S | The `security:sast` job has `allow_failure: true` **and** every command is suffixed with `|| echo ...`. Bandit and pip-audit results can never fail or even mark the pipeline; they are only visible by downloading artifacts. `docs/security/cve-2026-47265-suivi.md` exists, so CVEs are clearly being tracked manually instead. | Drop the `|| echo` on the human-readable runs, keep `allow_failure: true` for one release, then flip it to `false`. **Done (2026-09-01, cf. `CYBER_AUDIT.md` CS-R6-01)** : human-readable runs are blocking, `|| true` kept only on the JSON-artefact runs, and `allow_failure: false`. |
...
- [x] **F043** — Remove the `|| echo` suffixes from the human-readable bandit/pip-audit runs ; `allow_failure: false` flipped 2026-09-01 (`CYBER_AUDIT.md` CS-R6-01).
...
- ~~**`db/detections.db` committed to git.**~~ **Resolved 2026-09-01** (`CYBER_AUDIT.md` rév. 14): `git rm --cached db/detections.db`, `db/.gitkeep` added, `db/*.db` in `.gitignore` and `.dockerignore`. `init_db()` recreates the schema (`CREATE TABLE IF NOT EXISTS`).
...
8. ~~**JetPack version.**~~ **Resolved 2026-09-01.** Target fixed to **JetPack 7.2 / L4T r39.2 / Ubuntu 24.04** for the whole reServer J4012 fleet. README aligned, flash doc rewritten as `docs/deployment/flash-jetson-reserver-j4012-jetpack72.md` (with a référent sign-off table), offline L4T security-update + rollback procedure added (`docs/deployment/maj-l4t-hors-ligne.md`). See `CYBER_AUDIT.md` rév. 9 (`CS-1141-01/02`, `CS-123-03`). Remaining: formal homologation by the Stellantis référent.
