...
if [ "$UPGRADABLE" -eq 0 ]; then
    ok "Système à jour"; check_pass "Système à jour"
elif [ "$UPGRADABLE" -lt 10 ]; then
    warn "$UPGRADABLE paquets à mettre à jour"; check_warn "$UPGRADABLE paquets à mettre à jour"
...
if systemctl is-active --quiet unattended-upgrades 2>/dev/null || \
   [ -f /etc/apt/apt.conf.d/20auto-upgrades ]; then
    ok "Mises à jour automatiques actives"; check_pass "Mises à jour auto"
else
    warn "Mises à jour de sécurité automatiques non configurées"; check_warn "Mises à jour auto absentes"
...
if echo "$UFW_STATUS" | grep -q "actif\|active"; then
    ok "UFW actif"; check_pass "UFW actif"
    if sudo ufw status verbose 2>/dev/null | grep -q "deny (incoming)"; then
        ok "Default : deny incoming"; check_pass "Default deny incoming"
    else
    ...
    fail "UFW désactivé ou absent"; check_fail "UFW inactif"
...
# Filtre uniquement la colonne LOCAL ADDRESS (champ 4), pas le peer address
EXPOSED=$(sudo ss -tlnp 2>/dev/null | awk 'NR>1 && $4 ~ /^0\.0\.0\.0:[0-9]/ {print}' || true)

# Cross-référence UFW
UFW_DEFAULT_DENY=$(sudo ufw status verbose 2>/dev/null | grep -c "deny (incoming)" || echo 0)
UFW_ALLOWED=$(sudo ufw status 2>/dev/null | grep "ALLOW" | awk '{print $1}' | grep -oE '[0-9]+' || true)

if [ -z "$EXPOSED" ]; then
    ok "Aucun port exposé sur 0.0.0.0"; check_pass "Pas de port exposé"
else
    while IFS= read -r line; do
        PORT=$(echo "$line" | awk '{print $4}' | cut -d: -f2)
        PROCESS=$(echo "$line" | grep -oP 'users:\(\("\K[^"]+' || echo "inconnu")

        # Ignorer les ports VPN
        if echo "$PROCESS" | grep -qiE 'tailscale|netbird|wireguard'; then
            ok "Port $PORT ($PROCESS) — VPN interne, non exposé publiquement"
            check_pass "VPN port $PORT interne"
            continue
        fi

        UFW_PROTECTED=false
        if [ "$UFW_DEFAULT_DENY" -gt 0 ]; then UFW_PROTECTED=true; fi
        ...
        case "$PORT" in
            ...
            3389) fail "RDP (3389) exposé sur 0.0.0.0 — risque critique"
                  check_fail "RDP exposé internet"
                  fix "sudo ufw deny 3389 && restreindre à VPN/LAN" ;;
            5900|5901|5902|5999)
                  if [ "$UFW_PROTECTED" = true ]; then
                      ok "VNC ($PORT) sur 0.0.0.0 — bloqué par UFW default deny"
                      check_pass "VNC protégé UFW"
                      if echo "$UFW_ALLOWED" | grep -q "^${PORT}$"; then
                          warn "VNC ($PORT) a une règle ALLOW UFW — vérifier que la source est bien restreinte"
                          check_warn "VNC ALLOW UFW vérifier source"
                      fi
                  else
            ...
            8080|8000|8001|8002|8003|8004|8005|5050)
                  if [ "$UFW_PROTECTED" = true ]; then
                      ok "Port API ($PORT/$PROCESS) sur 0.0.0.0 — bloqué par UFW default deny"
                      check_pass "API $PORT protégée UFW"
                      warn "Défense en profondeur : binder sur 127.0.0.1:$PORT"
                      fix "Dans le code/config : écouter sur 127.0.0.1:$PORT au lieu de 0.0.0.0:$PORT"
                  else
                      warn "Port API ($PORT/$PROCESS) exposé — vérifier si intentionnel"
                      check_warn "API $PORT exposée"
                      fix "sudo ufw deny $PORT ou binder sur 127.0.0.1"
                  fi ;;
            *)    if [ "$UFW_PROTECTED" = true ]; then
                      warn "Port $PORT ($PROCESS) sur 0.0.0.0 — protégé UFW, binder sur 127.0.0.1 recommandé"
                      check_warn "Port $PORT non bindé localhost"
                      fix "Binder sur 127.0.0.1:$PORT pour défense en profondeur"
                  else
                      fail "Port $PORT ($PROCESS) exposé sur 0.0.0.0 — NON protégé !"
                      check_fail "Port $PORT exposé non protégé"
                      fix "sudo ufw deny $PORT ou binder sur 127.0.0.1:$PORT"
                  fi ;;
        esac
    done <<< "$EXPOSED"
fi

# Interface WAN (robuste sur Jetson/4G/PPP)
WAN_IFACE=$(ip route get 8.8.8.8 2>/dev/null | grep -oP 'dev \K\S+' | head -1)
if [ -n "$WAN_IFACE" ]; then
    WAN_IP=$(ip addr show "$WAN_IFACE" 2>/dev/null | grep "inet " | awk '{print $2}' | cut -d/ -f1 | head -1)
...
if command -v docker &>/dev/null; then
    ...
    if [ -z "$DOCKER_EXPOSED" ]; then
        ok "Aucun conteneur Docker exposé sur 0.0.0.0"; check_pass "Docker ports sécurisés"
    else
        fail "Conteneurs exposés sur 0.0.0.0 (contourne UFW) :"; check_fail "Docker bypass UFW"
    ...
    if [ -z "$PRIV" ]; then
        ok "Aucun conteneur en mode privilégié"; check_pass "Docker non privilégié"
    else
        fail "Conteneurs en mode privilégié (risque d'escalade) :"; check_fail "Docker privileged"
...
if systemctl is-active --quiet ssh 2>/dev/null || systemctl is-active --quiet sshd 2>/dev/null; then
    ...
    if echo "$ROOT_LOGIN" | grep -qi "no"; then
        ok "PermitRootLogin : no"; check_pass "SSH no root login"
    else
    ...
    if echo "$PASS_AUTH" | grep -qi "no"; then
        ok "PasswordAuthentication : no (clé uniquement)"; check_pass "SSH clé uniquement"
    else
        warn "PasswordAuthentication : yes — préférer l'auth par clé"; check_warn "SSH password auth"
    ...
    if [ "$SSH_PORT" != "22" ]; then
        ok "SSH sur port non standard : $SSH_PORT"; check_pass "SSH port non standard"
    else
        warn "SSH sur port 22 (standard) — envisager un port alternatif"; check_warn "SSH port 22"
    fi

    MAX_TRIES=$(grep -i "^MaxAuthTries" "$SSHD_CONFIG" 2>/dev/null | awk '{print $2}' || echo "6")
    if [ "$MAX_TRIES" -le 3 ] 2>/dev/null; then
        ok "MaxAuthTries : $MAX_TRIES"; check_pass "SSH MaxAuthTries"
    else
        warn "MaxAuthTries : $MAX_TRIES — recommandé ≤ 3"; check_warn "MaxAuthTries trop élevé"
...
if [ -z "$ROOT_USERS" ]; then
    ok "Aucun utilisateur UID 0 parasite (hors root)"; check_pass "UID 0 propre"
else
    fail "Utilisateurs avec UID 0 détectés : $ROOT_USERS"; check_fail "UID 0 parasite"
fi

NO_PASS=$(sudo awk -F: '($2 == "" || $2 == "!") && $1 != "nobody" {print $1}' /etc/shadow 2>/dev/null | head -5 || true)
if [ -z "$NO_PASS" ]; then
    ok "Tous les comptes ont un mot de passe"; check_pass "Mots de passe présents"
else
    fail "Comptes sans mot de passe : $NO_PASS"; check_fail "Comptes sans mot de passe"
fi
...
if [ -n "$LAST_FAIL" ]; then
    ...
    ok "Aucune tentative de connexion échouée récente"; check_pass "Pas de tentatives échouées"
fi
...
for svc in rpcbind avahi-daemon cups bluetooth telnet; do
    if systemctl is-active --quiet "$svc" 2>/dev/null; then
        ...
        ok "$svc : désactivé"; check_pass "$svc désactivé"
    fi
done
...
for param in "${!SYSCTL_CHECKS[@]}"; do
    ...
    if [ "$ACTUAL" = "$EXPECTED" ]; then
        ok "$param = $ACTUAL"; check_pass "sysctl $param"
    else
        warn "$param = $ACTUAL (recommandé : $EXPECTED)"; check_warn "sysctl $param non optimal"
...
if ip link show tailscale0 &>/dev/null 2>&1; then
    ...
    ok "Tailscale actif — IP : $TAILSCALE_IP"; check_pass "Tailscale VPN"
    VPN_FOUND=true
fi

if ip link show wt0 &>/dev/null 2>&1; then
    NETBIRD_IP=$(ip addr show wt0 | grep "inet " | awk '{print $2}' | cut -d/ -f1 || echo "?")
    ok "NetBird actif — IP : $NETBIRD_IP"; check_pass "NetBird VPN"
    VPN_FOUND=true
fi

if ip link show wg0 &>/dev/null 2>&1; then
    ok "WireGuard actif (wg0)"; check_pass "WireGuard VPN"
    VPN_FOUND=true
fi

if [ "$VPN_FOUND" = false ]; then
    warn "Aucun VPN détecté — recommandé si accès distant via internet"; check_warn "Pas de VPN"
