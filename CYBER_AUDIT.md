...
- **Date** : 2026-08-31 (révision 6 — doc de déploiement fusionnée/réalignée, RustDesk retiré du dépôt. Rév. 5 : chiffrement VNC forcé. Rév. 4 : RDP→TigerVNC. Rév. 3 : `CS-113-04` corrigé)
...
| CS-1143-04 | §1.1.4.3 | Protocoles de découverte automatique et de connexion à distance désactivés après vérification | Non conforme | Mineure | RustDesk retiré du dépôt (rév. 6 : `autostart/rustdesk-xhost.desktop` + `docs/deployment/script-deploiement.md` supprimés). Résiduel : option `install_vnc_jetson.sh --tailscale` (opt-in ; `tailscale0` bloqué par UFW par défaut ; doc = retirer pour le RUN) ; `scripts/security_audit.sh:336-344` (avahi/rpcbind/cups à contrôler sur cible) | Ne pas déployer avec `--tailscale` en RUN ; contrôler mDNS/Avahi/UPnP sur la cible (Hors dépôt) |
| CS-1143-05 | §1.1.4.3 | Besoin de communication externe exprimé au Plant IT Leader / Cybersecurity Leader dès le début du projet | Non conforme | Majeure | `src/bot_aiogram.py` (Telegram, encore dans le code) ; clé 4G de mise au point (RustDesk retiré du dépôt en rév. 6) | Décrire au PIL la **fenêtre de connexion 4G de mise au point** (durée, flux, retrait de la clé à la livraison) et attester le retrait du client Telegram pour l'état RUN |
...
| CS-127-02 | §1.2.7 | Aucun logiciel non nécessaire au fonctionnement ou non décrit au cahier des charges | Non conforme | Mineure | `Dockerfile:59` + venv copié (`cython==3.2.8`, `setuptools`, `wheel`) ; `pyproject.toml:19` (`setuptools>=80.8.0` en dépendance d'exécution) ; client Telegram encore dans le code | Retirer Cython/setuptools/wheel du venv de production ; retirer le client Telegram du livrable RUN (RustDesk/xrdp déjà retirés, rév. 6) |
...
3. *(Fait, révision 5-6 : script renommé `install_xrdp_jetson.sh` → `install_vnc_jetson.sh` ; doc `script-deploiement.md` supprimée, contenu fusionné dans `scripts-deploiement.md` sur le modèle autonome ; `autostart/rustdesk-xhost.desktop` supprimé.)*
...
- ~~**`xhost +local:root`** (`autostart/rustdesk-xhost.desktop`)~~ — **résolu (rév. 6)** : fichier supprimé avec RustDesk.
- **Documentation de déploiement** : `docs/deployment/script-deploiement.md` (RustDesk / xrdp) supprimé en révision 6, contenu utile (affichage headless, GDM3) fusionné dans `docs/deployment/scripts-deploiement.md`, aligné sur le modèle autonome + VNC chiffré. **Reste à corriger** : `README.md` (tableau des ports réseau décrivant `eth0` internet / supervision distante, section Telegram) — à réaligner avant présentation à l'auditeur Stellantis.
