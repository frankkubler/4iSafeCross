...
- **Date** : 2026-08-31 (révision 5 — chiffrement VNC forcé dans le script, renommé `install_vnc_jetson.sh`. Révision 4 : RDP→TigerVNC, `CS-1143-01` non bloquant. Révision 3 : `CS-113-04` corrigé)
...
**Ce que le dépôt ne permet pas de prouver** : la version de firmware/OS réellement installée sur le Jetson et son homologation par le référent technique Stellantis ; la présence des agents ACRONIS, CrowdStrike, StellarProtect ; le retrait effectif de la clé 4G et de RustDesk à l'issue de la mise au point ; l'application effective d'une politique de pare-feu hôte ; la conformité de la console de télémaintenance 4itec (STLA-CS_FOR_502) ; le respect des références matérielles au catalogue.
...
4. **Connecté ou autonome ?** — **Résolu (fournisseur, révision 2)** : autonome en exploitation, **sans connexion Internet**. Connexion **provisoire par clé 4G** pendant la mise au point sur site ; clé retirée ensuite. Accès résiduel : **local, RJ45, RDP**. Les fonctions de communication Internet présentes dans le code (bot Telegram, RustDesk, `pull` registry, option Tailscale) sont donc soit des vestiges de la phase de mise au point à retirer du livrable RUN, soit inertes en l'absence d'Internet. Verdict retenu : le dépôt contient toujours ce code actif et activable par configuration, d'où des constats maintenus mais rétrogradés en `Majeure`, et deux dérogations à formaliser pour la fenêtre 4G.
...
**Cycle de vie** : pendant la mise au point sur site, le boîtier est connecté à Internet par une clé 4G (téléchargement d'image, réglages, éventuellement supervision RustDesk/Telegram). À la livraison, la clé 4G est retirée : le boîtier devient autonome et le seul accès est un câble RJ45 point-à-point vers un PC de maintenance, en RDP. L'image de production est construite en multi-étapes (Docker) avec compilation Cython du code Python, et lancée en `privileged` + `network_mode: host` + `-v /dev:/dev`, en utilisateur root.
...
| CS-1143-01 | §1.1.4.3 | Aucun protocole interdit (HTTP, FTP, Telnet, RDP, SMBv1/2, SNMPv1/2, MQTT clair, MODBUS…) | Non conforme | Majeure | RDP/xrdp **abandonné** au profit de TigerVNC. VNC : `scripts/install_vnc_jetson.sh:120` force `-SecurityTypes X509Vnc,RA2ne` (session chiffrée, `VncAuth` refusé) — **à confirmer sur cible**. Résiduels : `run.py:32` (IHM en HTTP clair) ; `src/camera_manager.py:152` (`rtsp://` en clair) ; `docs/deployment/script-deploiement.md:15,27` (doc RDP périmée) | Vérifier sur cible que TigerVNC négocie X509Vnc/RA2ne ; placer l'IHM derrière TLS et la binder sur `eth2` ; isoler strictement le sous-réseau caméras ; corriger la doc `script-deploiement.md` |
...
| CS-1143-05 | §1.1.4.3 | Besoin de communication externe exprimé au Plant IT Leader / Cybersecurity Leader dès le début du projet | Non conforme | Majeure | `src/bot_aiogram.py` (Telegram) ; `docs/deployment/script-deploiement.md` (RustDesk) ; clé 4G de mise au point | Décrire au PIL la **fenêtre de connexion 4G de mise au point** (durée, flux, retrait de la clé à la livraison) et attester le retrait de Telegram/RustDesk pour l'état RUN |
...
| CS-127-02 | §1.2.7 | Aucun logiciel non nécessaire au fonctionnement ou non décrit au cahier des charges | Non conforme | Mineure | `Dockerfile:59` + venv copié (`cython==3.2.8`, `setuptools`, `wheel`) ; `pyproject.toml:19` (`setuptools>=80.8.0` en dépendance d'exécution) ; RustDesk/xrdp sur l'hôte | Retirer Cython/setuptools/wheel du venv de production ; retirer RustDesk/xrdp et le client Telegram du livrable RUN |
...
| CS-15-05 | §1.5 | Ces exigences valent aussi pour les machines fournisseur d'accès distant | Hors dépôt | — | Le PC qui ouvre les sessions RDP sur `eth2` (et RustDesk pendant la mise au point) est dans le périmètre | Inclure explicitement ce PC dans le périmètre STLA-CS_FOR_502 |
...
3. Corriger `docs/deployment/script-deploiement.md` (retirer xrdp/RDP). *(Renommage du script `install_xrdp_jetson.sh` → `install_vnc_jetson.sh` : fait, révision 5.)*
...
- **Documentation périmée** : `README.md` (ports réseau, section Telegram, `eth0` internet) et surtout `docs/deployment/script-deploiement.md` (RustDesk, **xrdp/RDP**, autologin) décrivent une machine connectée et un accès RDP qui **ne correspondent plus au déploiement réel** (autonome, accès TigerVNC chiffré). À réaligner impérativement avant présentation à l'auditeur Stellantis, sinon contradiction constatée. *(Le script a été renommé `install_xrdp_jetson.sh` → `install_vnc_jetson.sh` en révision 5 ; `script-deploiement.md` reste à corriger.)*
...
6. **Chiffrement de la session TigerVNC sur la cible** : le script force `-SecurityTypes X509Vnc,RA2ne` (rév. 5) — reste à confirmer sur le boîtier que le TigerVNC 1.12 installé négocie bien ces types et refuse `VncAuth`. La doc `docs/deployment/script-deploiement.md` (RDP/xrdp) reste à corriger.
...
- [ ] **TigerVNC** : vérifier sur cible que la session est chiffrée (`vncviewer -SecurityTypes VncAuth` doit échouer) ; corriger la doc RDP de `docs/deployment/script-deploiement.md` *(coût : faible)* — `CS-1143-01`
