...
- **Date** : 2026-09-01 (révision 8 — (a) IHM basculée en HTTPS : `waitress` lié à `127.0.0.1:5050` seul, reverse-proxy Caddy TLS sur `eth2` — `run.py`, `config/Caddyfile`, `scripts/caddy-4isafecross.service`, règle UFW 443 dans `install_vnc_jetson.sh` ; reste la vérification sur cible. (b) `CS-R6-01` : `aiohttp>=3.14.3` + `cryptography>=50.0.0`, `uv.lock` régénéré, `security:sast` passé en `allow_failure: false`. Rév. 7 : RustDesk repositionné comme accès distant provisoire de mise au point, comme la clé 4G. Rév. 6 : docs de déploiement fusionnées. Rév. 5 : chiffrement VNC forcé. Rév. 4 : RDP→TigerVNC. Rév. 3 : `CS-113-04` corrigé)
