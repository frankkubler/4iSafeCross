...
- `CS-1143-01` / `CS-R3-01` (protocoles — était `Bloquante`) — **rétrogradé en `Majeure` (révision 4)**. RDP/xrdp remplacé par **TigerVNC**. Révision 5 : le service force désormais `-SecurityTypes X509Vnc,RA2ne` (`scripts/install_vnc_jetson.sh:120`) — session chiffrée, `VncAuth`/`None` refusés ; reste à confirmer sur la cible. **Révision 8 : IHM basculée en HTTPS** — `waitress` lié à `127.0.0.1:5050` seul (`run.py`), reverse-proxy **Caddy** en frontal (`config/Caddyfile`, `tls internal`, `scripts/caddy-4isafecross.service`) exposant `https://192.168.3.122` sur `eth2` ; `install_vnc_jetson.sh` ouvre `443/tcp` au seul `192.168.3.0/24` et referme `5050`. Résiduel non corrigé : le transport RTSP caméras en clair. Reste la vérification sur cible (Caddy installé + service actif + certificat interne). Correctifs de niveau configuration.
...
| CS-1143-01 | §1.1.4.3 | Aucun protocole interdit (HTTP, FTP, Telnet, RDP, SMBv1/2, SNMPv1/2, MQTT clair, MODBUS…) | Non conforme | Majeure | RDP/xrdp **abandonné** au profit de TigerVNC. VNC : `scripts/install_vnc_jetson.sh:120` force `-SecurityTypes X509Vnc,RA2ne` (session chiffrée, `VncAuth` refusé) — **à confirmer sur cible**. **IHM en HTTPS (rév. 8)** : `run.py` lie `waitress` à `127.0.0.1:5050` seul, reverse-proxy Caddy TLS (`config/Caddyfile`) sur `eth2`. Résiduel : `src/camera_manager.py:152` (`rtsp://` en clair) | Vérifier sur cible : TigerVNC négocie X509Vnc/RA2ne ; Caddy actif, `5050` injoignable sur le réseau, certificat interne provisionné sur le PC de maintenance ; isoler strictement le sous-réseau caméras |
...
| CS-143-02 | §1.4.3 | Seuls les flux sécurisés nécessaires ouverts ; filtrage applicatif dès que possible | Non conforme | Majeure | **Rév. 8** : `run.py` lie `waitress` à `127.0.0.1:5050` seul (plus de `0.0.0.0`), `scripts/4isafecross.sh` aligné ; reverse-proxy Caddy TLS sur `eth2` ; `install_vnc_jetson.sh` ouvre `443/tcp` au seul `192.168.3.0/24` et referme `5050`. Résiduels : `docker-compose-*.yml` (`network_mode: host` — cf. `CS-1143-03`) ; auth applicative encore optionnelle (`CS-1144-01`) | Vérifier sur cible que `5050` est injoignable sur le réseau et que seul `443` répond sur `eth2` ; retirer `network_mode: host` ; rendre l'auth obligatoire. Pendant la fenêtre 4G, l'IHM n'est plus joignable depuis la clé cellulaire (liée à la boucle locale ; Caddy filtré par UFW sur `eth2`) |
...
**Impact** : réduit après la révision 8 — l'IHM ne transporte plus les identifiants HTTP Basic en clair sur le segment de maintenance. Reste le RTSP caméras en clair sur le sous-réseau vidéo dédié. La recette FOR_509 reste *zero-tolerance* sur le chiffrement.
