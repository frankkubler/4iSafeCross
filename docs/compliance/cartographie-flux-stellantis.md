...
- **Date** : 2026-09-01 · **Révision** : 17 (suit `CYBER_AUDIT.md` — rév. 17 : OO-01 caméras en clair, dérogation `CS-1143-01` ; tests VIGI sur cible ; segment caméras isolé outillé `scripts/setup-camera-net.sh`)
...
| OO-01 | Réception des flux vidéo H.264 des caméras (1 à 3) | Sous-réseau caméras **dédié** `192.168.0.0/24` (`br0` = `eth2`/`eth3`/`eth4`, ou switch PoE), isolé du reste ; aucune route par défaut | `CS-1143-01` — **dérogation requise** : les caméras TP-Link VIGI S485/S455 n'exposent pas de RTSPS/SRTP (impossibilité matérielle). Compensations : auth **Digest** (⚠️ Basic aussi accepté par la caméra), segment dédié isolé, équipements dédiés. `camera_manager` gère `SCHEME = rtsps` si des caméras compatibles sont installées |
...
- **Chiffrement des transports** : OO-01 (caméras) **en clair** — matériel VIGI S485/S455 sans RTSPS/SRTP → dérogation `CS-1143-01` + Digest (Basic aussi accepté) + sous-réseau caméras dédié et isolé ; les flux `127.0.0.1` (OO-02/03/04) ne traversent aucun média réseau ; tous les flux IT↔OT (IHM, VNC, SSH) sont chiffrés.
