> **Archive — approche abandonnée.**
> Le projet utilise GitLab CI/CD comme pipeline actif (voir [`../gitlab-ci-build.md`](../gitlab-ci-build.md)).
> Ce document décrit l'ancienne approche Nuitka avec GitHub Actions (exécutable standalone + tar.gz + SCP).
> Conservé pour référence si GitHub Actions + Nuitka est reconsidéré.
